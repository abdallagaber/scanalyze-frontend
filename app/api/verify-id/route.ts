...
// Define interface for Roboflow prediction object
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Processing image: ${imageFile.name}, Size: ${(
        imageFile.size / 1024
    ...
    console.log(
      ...
      JSON.stringify(roboflowRes.data).substring(0, 500) + "..."
    );

    const predictions: Prediction[] = roboflowRes.data.predictions || [];
    ...
    const isValid = predictions.some((pred: Prediction) => {
      // Check for various possible class names with lower confidence threshold
      const classNameLower = pred.class.toLowerCase();
      const isIdClass =
        classNameLower.includes("id") ||
        classNameLower.includes("egyp") ||
        classNameLower === "egyption-id";
    ...
    return NextResponse.json(
      { error: "Failed to get prediction from Roboflow" },
      { status: 500 }
    );
  }
}
